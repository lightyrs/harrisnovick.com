 ...
 */ (function ($) {
  ...
  $.fn.JamCity = function (options) {
    ...
    var defaults = {
      ...
      tooltips: false, // Do you want to use tooltips? Defaults to `true`. Variables: `true` or `false`.
          ...
          if (data.recenttracks.track) {
            ...
            var songLoop = function(){
              $.each(data.recenttracks.track, function (i, track) {
                ...
                //"<span class=\"jmc_vinyl_case_overlay\"></span>" +
                ((nowPlaying == true & settings.nowPlayingIcon == true) ? '<span class=\"jmc_nowplaying\"></span>' : '') + '<img src="' + ((displayArt == true) ? art : settings.noAlbumArtImg) + '" alt="' + song + ' by ' + artist + '" />' + "</a></li>");
                if (i == (settings.number - 1)) {
                  settings.onComplete.call(this);
                }
              });
